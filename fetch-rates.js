  ...
  try {
    ...
    const tableData = await page.evaluate(() => {
      ...
      for (const row of rows) {
        ...
        if (text.includes('wise') || text.includes('remitly') || text.includes('sendwave')) {
          const cells = Array.from(row.querySelectorAll('td'));
          if (cells.length > 1) {
            const name = cells[0].innerText.trim();
            const rateMatch = cells[1].innerText.match(/[0-9,.]+/);
            if (rateMatch) {
              result[name.toLowerCase()] = parseFloat(rateMatch[0].replace(/,/g, ''));
            }
          }
        }
      }
    ...
    if (tableData && Object.keys(tableData).length > 0) {
      console.log('Extracted Compa Rates:', tableData);
      for (const [provider, rate] of Object.entries(tableData)) {
        if (rate) {
          await saveRate(`compa_${provider.replace(/\\s+/g, '_')}`, rate, rate + 50);
        }
      }
    }
